...
exports.buildIndexTree = (folders) => {
    const files = getFiles(folders)
        .filter(f => config_1.config.ignore ? !micromatch_1.default.isMatch(f, config_1.config.ignore) : true);
    console.log(files.join('\n'));
    return (files
        .filter(file => micromatch_1.default.isMatch(path_1.default.basename(file), config_1.INDEX_GLOB))
        .filter(indexPath => {
